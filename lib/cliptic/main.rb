module Cliptic
  module Main
    ...
    module Fetch
      class Request
        ...
        def fetch_with_http
          ...
          http.use_ssl = true
          http.verify_mode = OpenSSL::SSL::VERIFY_NONE  # Similar to curl's ssl_verify_peer = false
      ...
      class Cache < Request
        # Windows-compatible cache path
        Path = if WINDOWS
          "#{ENV['LOCALAPPDATA']}\\cliptic\\cache"
        else
    ...
    module Puzzle
      # Puzzle classes remain the same...
      class Puzzle
        include Fetch
    ...
    module Player
      ...
      class Controller
        ...
        def is_arrow_key?(char)
          (258..261).cover?(char) || 
          (WINDOWS && [72, 80, 75, 77].include?(char))  # Windows arrow key codes
        end
        ...
        private
        def normalize_windows_key(char)
          # Handle Windows-specific key codes
          return char unless WINDOWS
          
          case char
          when 8   then 127  # Backspace
          when 224 then game.user_input  # Extended key prefix
          else char
          end
        end
      end
